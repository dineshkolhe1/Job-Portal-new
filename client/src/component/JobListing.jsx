...
const JobListing = () => {
  ...
  useEffect(() => {
    const matchesCategory = (job) =>
      ...
      selectedCategories.some(
        (c) =>
          c.toLowerCase().trim() === (job.category || "").toLowerCase().trim()
      );
    ...
    const matchesLocation = (job) =>
      ...
      selectedLocation.some(
        (l) =>
          l.toLowerCase().trim() === (job.location || "").toLowerCase().trim()
      );
  ...
  useEffect(() => {
    if (jobs && jobs.length > 0) {
      setFilterJobs(jobs.slice().reverse());
    }
  }, [jobs]);
  ...
  return (
    <div className="container 2xl:px-20 mx-auto flex flex-col lg:flex-row max-lg:space-y-8 py-8">
      ...
      <section className="w-full lg:w-3/4 text-gray-800 max-lg:px-4">
        ...
        <p className="mb-8">
          Apne hunar, mehnat aur jazbe ke hisaab se kaam paaye — yahin, apne
          sheher mein.
        </p>
